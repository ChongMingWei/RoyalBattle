...
Earth_element::Earth_element(QGraphicsItem *parent)
{
    //set the health
    health =1;

    //set graph
    setPixmap(QPixmap(":/image/champions2/earth_element.png"));
    setPos(0,0);
...
void Earth_element::move_forward()
{
    ...
    for(int i = 0,n=colliding_items.size();i<n;++i)
    {
        ...
        else if (typeid(*(colliding_items[i])) == typeid(Autocannon))
        {
            --health;
        }
        else if (typeid(*(colliding_items[i])) == typeid(Machinegun))
        {
            --health;
        }
        if(health <= 0)
